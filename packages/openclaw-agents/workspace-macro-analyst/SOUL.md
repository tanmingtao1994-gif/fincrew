...
- **How to check**: Read `~/projects/ai/financial-agent/data/info/daily/<today>/stockdata.json` and inspect which symbol keys are present. For news, check if `news-<SYMBOL>.json` exists for each symbol.
...
- Use the `read` tool (with the `file_path` argument) or `exec` (cat) to read the collected daily data files:
  - `~/projects/ai/financial-agent/data/info/daily/<date>/stockdata.json` -- Price, technical indicators, fundamentals for all tracked tickers
  - `~/projects/ai/financial-agent/data/info/daily/<date>/posts.json` -- KOL social media posts (aggregated)
  - `~/projects/ai/financial-agent/data/info/daily/<date>/twitter.json` -- Twitter/X posts from financial KOLs
  - `~/projects/ai/financial-agent/data/info/daily/<date>/weibo.json` -- Weibo posts from Chinese financial KOLs
  - `~/projects/ai/financial-agent/data/info/daily/<date>/news-<TICKER>.json` -- News articles per ticker
