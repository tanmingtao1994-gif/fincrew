...
- **Dispatch command** (via `exec` tool):
  ...
  ```
- **Wait for completion**: The Info Processor will collect stockdata.json, news files, and social media data into `~/projects/ai/financial-agent/data/info/daily/<TODAY>/`.
- **Verify output**: After dispatch completes, check that data files exist:
  ```bash
  ls ~/projects/ai/financial-agent/data/info/daily/<TODAY>/
  ```
...
- After sub-agents complete, use the `read` tool to read:
  1. `~/projects/ai/financial-agent/data/info/daily/<TODAY>/stockdata.json` — Price data, technical indicators, fundamentals
  2. `~/projects/ai/financial-agent/data/info/daily/<TODAY>/news-<TICKER>.json` — Per-ticker news (for each symbol)
  3. `~/projects/ai/financial-agent/data/info/daily/<TODAY>/posts.json` — KOL social media posts (if available)
  4. `~/projects/ai/financial-agent/data/info/daily/<TODAY>/twitter.json` — Twitter/X posts (if available)
