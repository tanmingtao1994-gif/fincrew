...
- **Read trading plans and market data**: Use the `read` tool (with `file_path`) or `exec` (cat/ls) to read:
  1. `~/projects/ai/financial-agent/data/info/daily/<date>/stockdata.json` — Current market data for comparison
  2. `~/projects/ai/financial-agent/data/memory/memory.json` — Historical trade records and lessons
  3. Any trading plan files that may exist in `data/info/daily/<date>/` directory
- **List available dates**: `ls ~/projects/ai/financial-agent/data/info/daily/` to see which dates have data
