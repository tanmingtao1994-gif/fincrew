...
export interface SearchHotKeywordsInput {
  sources?: ('twitter' | 'weibo' | 'youtube')[];
  topN?: number;
  date?: string;
}

/**
 * Skill to search for hot keywords in collected social data.
 */
export async function searchHotKeywords(input: SearchHotKeywordsInput): Promise<HotKeyword[]> {
  return withErrorHandling(async () => {
    const { sources = ['twitter', 'weibo', 'youtube'], topN = 10, date } = input;
    const targetDate = date || new Date().toISOString().split('T')[0];

    // 1. Collect data (Ensure fresh data if needed, or rely on existing)
    // For now, we assume data is collected or we trigger collection
    // To be safe and ensure data availability, we trigger collection
    await stockRichAdapter.collect(targetDate);

    // 2. Get posts
    // We need to read from the files generated by stockRichAdapter (which uses stock_rich logic)
    // stock_rich saves to data/daily/{date}/{platform}.json
    const posts: any[] = [];
    
    for (const p of sources) {
        // Use readDailyData from stock_rich utils (now in root)
        // We need to import it properly. 
        // Note: stock_rich/utils/cache.ts was refactored to use process.cwd()
        const data = await readDailyData<any[]>(targetDate, p);
    ...
    return sorted;
  }, 'searchHotKeywords', ErrorCode.EXTERNAL_API_ERROR);
}
