...
- **How to check**: Read `~/projects/ai/financial-agent/data/daily/<today>/stockdata.json` via the `read` tool (with `file_path`) or `exec` (cat).
...
- Use the `read` tool (with `file_path`) to read:
  - `~/projects/ai/financial-agent/data/daily/<today>/stockdata.json` — Main data source
